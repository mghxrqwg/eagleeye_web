    ...
    $(function() {
        ...
        $(document).on('click', '.popover-title .close', function(e) {
            ...
            if ($target.hasClass('nohide')) {
                $target.parents('div.popover').remove();
            } else {
                var $popover = $target.closest('.popover').prev();
        ...
        var navstatus = window.localStorage.getItem('navstatus');
        window.localStorage && navstatus && $('#nav').addClass('nav-xs') && $('#navxs_btn').addClass('active');
        $(document).on('click', '[data-toggle^="class"]', function(e) {
            ...
            if ($tmp == 'nav-xs' && window.localStorage) {
                if ($this.hasClass('active')) {
                    window.localStorage.setItem('navstatus', 'nav-xs');
                } else {
                    window.localStorage.setItem('navstatus', '');
                }
            } 
        });
        ...
        $(document).on('click', '.nav-primary a', function(e) {
        ...
        }).on('spaHashChange', function (e, module) {
            var $this = $('#left_nav_bar a[href^="#/' + module + '"]');
            if ($this.hasClass('active')) {
                return;
            }
            var $active = null;
            $this.parents('li').each(function () {
                ...
                $li.siblings('.active').each(function () {
                    ...
                    $item.find('> ul:visible').slideUp(200);
